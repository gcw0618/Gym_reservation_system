layui.define(['element'], function(exports) {
	...
	function createStyle(ulClassName, width) {
		var style =
			'.{name} {position: absolute;width: {width_}px;z-index: 9999;display: none;background-color: #ffffff;box-shadow:0px 0px 10px #000;color: #333;border: 1px solid #eee;border-radius: 2px;cursor: pointer;}.{name} li {text-align: center;display: block;height: 30px;line-height: 32px;padding: 0px 5px 0 5px;}.{name} li:hover {background-color: #0a7a69;color: #fff;}'
			.format({
				name: ulClassName,
				width_: width
			});
	...
	RIGHTMENUMOD.prototype.render = function(opt) {
		...
		var defaultNavArr = [{
			...
			{
				eventName: 'closeothersbutpin',
				title: '关闭其它但固定'
			},
			{
				eventName: 'closeleftbutpin',
				title: '关闭左侧标签但固定'
			},
			{
				eventName: 'closerightbutpin',
				title: '关闭右侧标签但固定'
			}
		];
		opt = opt || {};
	...
	function registClickMidCloseTab(isClose, rightMenuConfig) {
		...
		$("#" + rightmenuObj.filter).on('mousedown', 'li', function(e) {
			...
			if (rightMenuConfig.pinTabTitles && rightMenuConfig.pinTabTitles.indexOf(currentActiveTabTitle) != -1 ||
				currentActiveTabTitle == undefined) { //特殊ID，弹出默认的右键菜单
				return true;
			}
			if (rightMenuConfig.pintabIDs && rightMenuConfig.pintabIDs.indexOf(currentActiveTabID) != -1 ||
				currentActiveTabID == undefined) { //特殊ID，弹出默认的右键菜单
				return true;
			}
			if (e.which != 2) {
				return true;
		...
		$.each(rightMenuConfig.navArr, function(index, conf) {
			if (conf.eventName === 'line')
				...
				.format({
					...
					icon: conf.icon ? conf.icon : "",
					title: conf.title
				});
		})
		var tmpHtml = '<ul class="{className}">{liStr} </ul>'.format({
			liStr: li,
			className: rightMenuConfig.filter
		})
	...
	function _CustomRightClick(rightMenuConfig) {
		...
		$("#" + rightmenuObj.filter).on('contextmenu', 'li', function(e) {
			...
			if (rightMenuConfig.pinTabTitles && rightMenuConfig.pinTabTitles.indexOf(currentActiveTabTitle) != -1 ||
				currentActiveTabTitle == undefined) { //特殊ID，不弹出右键菜单
				return false;
			}
			if (rightMenuConfig.pintabIDs && rightMenuConfig.pintabIDs.indexOf(currentActiveTabID) != -1 ||
				currentActiveTabID == undefined) { //特殊ID，不弹出右键菜单
				return false;
			}
		...
		$('.' + rightMenuConfig.filter + ' li').click(function(e) {
			...
			switch ($(this).attr("data-type")) {
				...
				default:
					...
					rightMenuConfig.registMethod[$(this).attr("data-type")](currentActiveTabID, currentTitle, rightMenuConfig.container,
						$(this)[0]);
			}
			$('.rightmenu').hide();
		})
